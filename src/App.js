    ...
    <div className='employee'>
      ...
      <ul>
        {employees.map(employee => (
          <li key={employee.id} style={{ color: employee.disabled ? 'gray' : 'black' }}>
            {`${employee.first_name} ${employee.last_name}, Age: ${employee.age}`}
            <button onClick={() => addToTeam(employee)} disabled={employee.disabled}>ADD</button>
          </li>
        ))}
      </ul>
    </div>
...
const TeamList = ({ team, removeFromTeam, sortTeam }) => {
  const averageAge = team.length > 0 ? team.reduce((acc, curr) => acc + curr.age, 0) / team.length : 0;

  return (
    <div className='team'>
      <h2>Team Members</h2>
      <button onClick={sortTeam}>SORT BY AGE</button>
...
const App = () => {
  ...
  const addToTeam = employee => {
    const updatedEmployeesList = employeesList.map(emp =>
      emp.id === employee.id ? { ...emp, disabled: true } : emp
    );
  ...
  const removeFromTeam = employee => {
    const updatedTeamMembers = teamMembers.filter(member => member.id !== employee.id);
    const updatedEmployeesList = employeesList.map(emp =>
      emp.id === employee.id ? { ...emp, disabled: false } : emp
    );
  ...
  return (
    <div style={{ display: 'flex' }}>
      <EmployeeList employees={employeesList} addToTeam={addToTeam} />
      <TeamList team={teamMembers} removeFromTeam={removeFromTeam} sortTeam={sortTeam} />
    </div>
  );
};
